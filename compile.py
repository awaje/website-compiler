...
def main():
    ...
    go_through_dir(Path(sys.argv[1]), sys.argv[2], Path(sys.argv[3]))


def go_through_dir(dir_: Path, base_path: str, base_output: Path, is_first: bool = True):
    if is_first:
    ...
    for file in dir_.iterdir():
        if file.is_file():
            if config["prefixes"]["raw"] in file.name:
                ...
                output.close()
                print("RAW:      " + base_path + "/" + file.name[5:] + " " + str(file.absolute()))
                ...
                print("LINK:     " + base_path + "/" + file.name[6:] + " " + str(file.absolute()))
            else:
            ...
            print("DIR:      " + base_path + "/" + file.name + "/" + " " + str(file.absolute()))
            ...
            go_through_dir(file, base_path + "/" + file.name, base_output / file.name, False)


def cat_listing(path: Path, base_path: str):
    return put_in_body([
        ls(path.parent, base_path),
        cat(path, base_path),
        prompt(base_path)
    ], base_path)
...
def dir_listing(path: Path, base_path: str):
    return put_in_body([
        ls(Path(path), base_path),
        prompt(base_path)
    ], base_path)


def put_in_body(elements: [str], base_path: str):
...
def ls(path: Path, base_path: str) -> str:
    ...
    for file in path.iterdir():
        if file.is_file() and (
                file.name not in config["ignore"] and
                not prefix_in_name(file.name)
        ):
            files.append([
                "-rw-r--r--",
                "1",
                ...
                datetime.datetime.fromtimestamp(file.stat().st_mtime).strftime("%b"),
    ...
    for row in files:
        ...
        for col in range(len(row) - 1):
            output += "&nbsp;" * (lengths[col] - len(row[col])) + html.escape(row[col]) + " "
        if row[0][0] == 'd':
            if row[-1] == ".":
